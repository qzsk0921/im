<template v-if="chatProps.liveUser">
  <div id="live-chat-msg-div">
    <div class="chat-msg-item" v-for="(textMsg, i) in msgData" :key="i">
        <span class="msg-item text-msg" v-if="((textMsg.type == 1 || textMsg.type == 16) && textMsg.objectName == 'HEP:TxtMsg')">
            <img v-if="textMsg.user.userVip" class="user-level" :src="'../../static/images/userLevel/pic_caifu_lv'+textMsg.user.userVip+'@2x.png'" alt="">
            <span class="user-name-span" v-html="textMsg.user.name + ' : '"></span><span class="line-height-span" :class="{'barrage': textMsg.type == 16}" v-html="textMsg.content"></span>
        </span>
        <span class="msg-item in-live-room" v-else-if="(textMsg.type == 3 && textMsg.objectName == 'HEP:TxtMsg')" >
            <img v-if="textMsg.user.userVip" class="user-level" :src="'../../static/images/userLevel/pic_caifu_lv'+textMsg.user.userVip+'@2x.png'" alt="">
            <span class="line-height-span" v-html="textMsg.content"></span>
        </span>
        <span class="msg-item in-live-room" v-else-if="((textMsg.type == 2 || textMsg.type == 5) && textMsg.objectName == 'HEP:TxtMsg')" >
            <span class="line-height-span" v-html="textMsg.content"></span>
        </span>
...
export default {
  ...
  data: function () {
        return {
            ...
            userData: {
                ...
                userVip: (this.$common.getLocalStorage('level') ) ? this.$common.getLocalStorage('level') : 0,
            }
        }
  },
  created: function () {
  ...
  watch: {
      msgData: function () {
        ...
        this.$nextTick(() => {
            let el = document.getElementById('live-chat-msg-div').querySelector('div:last-child')
  ...
  methods: {
      initChatDate: function () {
            ...
            eventBus.$on('sendGif', data => {
                ...
                //  console.log(data);
                this.sendIMMessage(messageName, objName, propertysNames, msgContent, this.giftMsg);
            })
            ...
            eventBus.$on('sendTextMsg', data => {
                ...
                this.sendIMMessage(messageName, objName, propertysNames, msgContent, this.sendTextMsgRes);
            })
    ...
    // 直播间公告
      welcomeMsg: function () {
          let data = {
              content: '{"content":"合拍直播: '+ this.chatProps.liveUser.welcome_msg +'","type":3,"price":0}',
    ...
    // 发送IM聊天消息
    sendIMMessage: function (messageName, objName, propertysNames, msgContent, callBack) {     // 发送IM消息
